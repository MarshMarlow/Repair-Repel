...
public class ObjectGrabbable : MonoBehaviour
{
    ...
    private DistanceGrabInteractor left;
    private DistanceGrabInteractor right;
    
    public void Awake()
    {
        // playerTransform = GameObject.FindGameObjectWithTag("MainCamera").transform;
        objectRigidbody = GetComponent<Rigidbody>();
        left = GameObject.Find("LeftController").GetComponentInChildren<DistanceGrabInteractor>();
        right = GameObject.Find("RightController").GetComponentInChildren<DistanceGrabInteractor>();
    }

    public void Grab()
    {
        objectGrabPointTransform = isHeldByLeft() ? left.transform : right.transform;
    ...
    public bool isHeldByLeft()
    {
        return left.HasSelectedInteractable && left.SelectedInteractable.transform.parent == transform;
    }
    public bool isHeldByRight()
    {
        return right.HasSelectedInteractable && right.SelectedInteractable.transform.parent == transform;
    }
}
